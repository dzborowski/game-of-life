...
GridSize::GridSize(GridSize &&other) noexcept {
    std::cout << "Before move constructor address: width - " << &other.width << " height - " << &other.height
              << std::endl;

    this->width = std::move(other.width);
    this->height = std::move(other.height);

    std::cout << "After move constructor address: width - " << &other.width << " height - " << &other.height
              << std::endl;
}
